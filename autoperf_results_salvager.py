...
def generate_dataset(summaries_dirpath: str = "", truncation_percent: int = 0) -> Optional[str]:
    """
    ...
    filename = os.path.join(os.path.dirname(summaries_dirpath), filename).replace("summarised_data", "datasets")
...
def main():
    ...
    for EXPERIMENT in CONFIG:
        ...
        all_filepaths = []
        for path in EXPERIMENT["paths"]:
            all_filepaths += [os.path.join(path, file) for file in os.listdir(path) if file.endswith(".csv")]
        ...
        combined_folder = os.path.join(os.path.dirname(EXPERIMENT['paths'][0]), f"salvaged_{EXPERIMENT['name']}")
        os.makedirs(combined_folder, exist_ok=True)
        for filepath in track(unique_filepaths, description=f"Combining files for {EXPERIMENT['name']}"):
