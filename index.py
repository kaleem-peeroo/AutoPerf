...
try:
    uses_random_generation = config['random_generation']
except KeyError:
    # ? Random generation is disabled.
    uses_random_generation = False

def main(config, buffer_multiple, uses_random_generation):
    
    if uses_random_generation:
        
        # ? Look for generated_test_count.
        try:
            total_test_count = config['generated_test_count']
        
            # ? Check that the total test count is greater than 0.
            if total_test_count <= 0:
                console.print(f"Generated test count is less than or equal to 0 in the config file: {config}.", style="bold red")
                sys.exit()
                
        except KeyError:
            # ? generated_test_count is not in the config file.
            console.print(f"Couldn't find the generated_test_count field in the config file: {config}", style="bold red")
            sys.exit()
        except ValueError:            
            # ? generated_test_count is not an integer.
            console.print(f"Generated test count is not an integer in the config file: {config}.", style="bold red")
            sys.exit()
        ...
        console.print(f"Running {camp_name} in [bold blue]Random Generation Mode[/bold blue].", style="bold green")
        ...
        for i in range(int(total_test_count)):
            log_debug(f"Generating test {i + 1} of {total_test_count}...")
            
            # ? Generate a random combination that hasn't been generated before.
            while True:
                combination = generate_random_combination(config['settings'])
                test_title = get_test_title_from_combination(combination)
                
                if test_title not in combination_histories:
                    combination_histories.append(test_title)
                    break

                log_debug(f"Generated {test_title} but it already exists. Generating another combination...")
                
            log_debug(f"Generated {test_title}.")
...
main(config, buffer_multiple, uses_random_generation)