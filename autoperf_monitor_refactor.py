...
def run_command_via_ssh(
    ...
    if command == "":
        return None, f"No command passed."
    ...
    if not ping_machine(machine_ip):
        return None, f"Couldn't ping {machine_name} ({machine_ip})."

    if not check_ssh_connection(machine_config):
        return None, f"Couldn't SSH into {machine_name} ({machine_ip})."
...
def calculate_target_test_count_for_experiments(
    config: Dict = {}
) -> Tuple[Optional[Dict], Optional[str]]:
    """
...
def calculate_duration(
    ...
    if start_timestamp == "":
        return None, "No start timestamp passed."

    if end_timestamp == "":
        return None, "No end timestamp passed."
...
def main(sys_args: list[str] = []) -> None:
    ...
    for MACHINE_CONFIG in CONFIG:
        ...
        with Live(table, refresh_per_second=8) as live:
            is_pingable, error = check_connection(MACHINE_CONFIG)
            if not is_pingable or error:
                logger.error(
                    f"Couldn't ping {MACHINE_CONFIG['name']} ({MACHINE_CONFIG['ip']}). {error}"
                )
                continue

            ap_config = get_ap_config_from_machine(MACHINE_CONFIG)

            for campaign in ap_config:
                campaign_data = {}
                ...
                campaign, error = get_ess_df(MACHINE_CONFIG, campaign)
                if error:
                ...
                elapsed_time, error = get_elapsed_time(campaign)
                if error:
                    campaign_data['Elapsed\n-----\nExpected\nTime'] = "-"
                    updated_table = create_table(table_data)
                    live.update(updated_table)
    ...
    #
    #     ongoing_info, error = get_ongoing_info_from_machine(MACHINE_CONFIG)
    #     if error:
    #         machine_name = MACHINE_CONFIG['name']
    #         machine_ip = MACHINE_CONFIG
    #         logger.error(
    #             f"Couldn't get ongoing info from {machine_name} ({machine_ip}) {error}."
    #         )
    #         continue
    #
    ...
    #
    #     duration, error = calculate_duration(start_timestamp, end_timestamp)
    #     if error:
    #         logger.error(
    #             f"Couldn't calculate duration: {error}"
    #         )
    #         return None
    #
