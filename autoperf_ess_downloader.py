...
def download_ess_from_machine(machine: Dict = {}, status: Console.status = None):
    if machine == {}:
        logger.error(
            f"No machine config passed."
        )
    ...
    if get_esses_output is None:
        logger.error(
            f"Couldn't get ESSes from {machine['name']} ({machine['ip']})."
        )
        return None

    os.makedirs(f"ess/{machine['name']}", exist_ok=True)

    esses = get_esses_output.split()

    console.print(f"Found {len(esses)} ESSes on {machine['name']}.", style="bold white")

    for ess in esses:
        i = esses.index(ess)
        i += 1
        counter_string = f"[{i}/{len(esses)}]"

        get_file_size_command = f"du -h {ess}"
        ...
        if get_file_size_output is None:
            logger.error(
                f"Couldn't get file size of {ess} from {machine['name']}"
            )
            return None

        file_size = get_file_size_output.split()[0]
        status.update(f"{counter_string} Downloading {ess} ({file_size}B) from {machine['name']}...")

        download_command = f"scp -i {machine['ssh_key_path']} {machine['username']}@{machine['ip']}:{ess} ./ess/{machine['name']}/"
        ...
        if download_output.returncode != 0:
            logger.error(
                f"Error downloading {ess} from {machine['name']}: {stderr}"
            )
            return None

        console.print(f"Downloaded {os.path.basename(ess)} ({file_size}B) from {machine['name']}.", style="bold green")
...
def main(sys_args: list[str] = []) -> None:
    ...
    for MACHINE_CONFIG in CONFIG:
        ...
        with console.status(f"Downloading data from {machine_name}...") as status:
            download_ess_from_machine(MACHINE_CONFIG, status)
