...
def download_logs_from_machine(machine: Dict = {}, status: Console.status = None):
    if machine == {}:
        logger.error(
            f"No machine config passed."
        )
    ...
    for log in logs:
        i = logs.index(log)
        i += 1
        counter_string = f"[{i}/{len(logs)}]"

        get_file_size_command = f"du -h {log}"
        ...
        if get_file_size_output is None:
            logger.error(
                f"Couldn't get file size of {log} from {machine['name']}"
            )
            return None
        ...
        if download_output.returncode != 0:
            logger.error(
                f"Error downloading {log} from {machine['name']}: {stderr}"
            )
            return None

        console.print(f"Downloaded {os.path.basename(log)} ({file_size}B) from {machine['name']}.", style="bold green")
