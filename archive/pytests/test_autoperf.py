...
def generate_random_ess_row():
    ...
    random_test_name = (
        f"{duration_secs}SEC_{datalen_bytes}B_{pub_count}PUB_{sub_count}SUB_"
    )
    random_test_name += (
        f"{'REL' if use_reliable else 'BE'}_{'MC' if use_multicast else 'UC'}_"
    )
...
class TestAutoPerf(unittest.TestCase):
    def setUp(self):
        ...
        os.system(
            "cp -r ./pytests/summarised_data/Multicast* ./output/summarised_data/"
        )

    def tearDown(self):
        # Delete copied folders from ./summarised_data
        os.system(
            "cp -r ./output/summarised_data/Multicast* ./pytests/summarised_data/"
        )
    ...
    def test_read_config(self):
        ...
        for config_path in config_paths_that_return_none:
            self.assertEqual(ap.read_config(config_path), None)
    ...
    def test_get_ess_df(self):
        ...
        self.assertEqual(len(ess_df.columns), len(required_columns))

        self.assertEqual(list(ess_df.columns), required_columns)
    ...
    def test_get_valid_dirname(self):
        ...
        for index, _ in enumerate(test_inputs):
            ...
            self.assertEqual(ap.get_valid_dirname(test_input), test_output)
    ...
    def test_get_test_name_from_combination_dict(self):
        ...
        self.assertEqual(test_name, "10SEC_100B_10PUB_10SUB_REL_UC_0DUR_100LC")
        ...
        test_name = ap.get_test_name_from_combination_dict(test_dict)
        self.assertEqual(test_name, "10SEC_100B_10PUB_10SUB_BE_UC_0DUR_100LC")
    ...
    def test_get_next_test_from_ess(self):
        ...
        self.assertEqual(ap.get_next_test_from_ess(None), None)

        self.assertEqual(ap.get_next_test_from_ess(pd.DataFrame()), {})

    def test_have_last_n_tests_failed(self):
        ess_df = ap.get_ess_df("./pytests/ess/good_ess_1.csv")
        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 3), True)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 5), False)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 0), False)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, -1), None)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 100), False)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 50), False)

        self.assertEqual(ap.have_last_n_tests_failed(ess_df, 49), False)

    def test_get_buffer_duration_secs_from_test_duration_secs(self):
        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(10), 30)

        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(60), 30)

        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(120), 30)

        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(0), None)

        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(-1), None)

        self.assertEqual(ap.get_buffer_duration_secs_from_test_duration_secs(1200), 60)
    ...
    def test_get_if_pcg(self):
        self.assertEqual(ap.get_if_pcg(None), None)
    ...
    def test_validate_dict_using_keys(self):
        ...
        self.assertEqual(
            ap.validate_dict_using_keys(["one", "two", "three"], ["one", "two"]), False
        )

        self.assertEqual(ap.validate_dict_using_keys(["one"], ["one", "two"]), False)

    def test_get_difference_between_lists(self):
        self.assertEqual(ap.get_difference_between_lists([1, 2, 3], [1, 2, 3]), [])

        self.assertEqual(ap.get_difference_between_lists([1, 3], [1, 2, 3]), [2])

        self.assertEqual(ap.get_difference_between_lists([], [1, 2, 3]), [1, 2, 3])

    def test_get_longer_list(self):
        test_cases = [
            {"list1": [1, 2, 3], "list2": [], "expected_output": [1, 2, 3]},
            {"list1": [], "list2": [1, 2, 3], "expected_output": [1, 2, 3]},
            {"list1": [1, 2, 3], "list2": [1, 2, 3], "expected_output": [1, 2, 3]},
            {
                "list1": [1, 2, 3],
                "list2": [1, 2, 3, 4],
        ...
        for test_case in test_cases:
            ...
            self.assertEqual(ap.get_longer_list(list1, list2), expected_output)

    def test_get_shorter_list(self):
        test_cases = [
            {"list1": [1, 2, 3], "list2": [], "expected_output": []},
            {"list1": [], "list2": [1, 2, 3], "expected_output": []},
            {"list1": [1, 2, 3], "list2": [1, 2, 3], "expected_output": [1, 2, 3]},
            {"list1": [1, 2, 3], "list2": [1, 2, 3, 4], "expected_output": [1, 2, 3]},
            {"list1": [], "list2": [], "expected_output": []},
        ]
        ...
        for test_case in test_cases:
            ...
            list2 = test_case["list2"]
            expected_output = test_case["expected_output"]
            self.assertEqual(ap.get_shorter_list(list1, list2), expected_output)

    def test_ping_machine(self):
        self.assertEqual(ap.ping_machine("www.google.com"), True)
    ...
    def test_distribute_scripts_to_machines(self):
        ...
        self.assertEqual(distributed_scripts, None)
    ...
    def test_get_machines_by_type(self):
        ...
        self.assertEqual(len(pub_machines), 1)
        self.assertEqual(len(sub_machines), 1)

        self.assertEqual(pub_machines[0]["machine_name"], "p1")
        self.assertEqual(sub_machines[0]["machine_name"], "p2")
